...
test_that("block_entropy on single series", {
  expect_equal(block_entropy(c(1, 1, 0, 0, 1, 0, 0, 1),
                           k = 2, local = !T), 1.950212, tolerance = 1e-6)
  expect_equal(block_entropy(c(1, 0, 0, 0, 0, 0, 0, 0, 0),
                           k = 2, local = !T), 0.543564, tolerance = 1e-6)
  expect_equal(block_entropy(c(0, 0, 1, 1, 1, 1, 0, 0, 0),
                           k = 2, local = !T), 1.811278, tolerance = 1e-6)
  expect_equal(block_entropy(c(1, 0, 0, 0, 0, 0, 0, 1, 1),
                           k = 2, local = !T), 1.548795, tolerance = 1e-6)
  expect_equal(block_entropy(c(3, 3, 3, 2, 1, 0, 0, 0, 1),
                           k = 2, local = !T), 2.50000, tolerance = 1e-6)
  expect_equal(block_entropy(c(2, 2, 3, 3, 3, 3, 2, 1, 0),
                           k = 2, local = !T), 2.40564, tolerance = 1e-6)
  expect_equal(block_entropy(c(2, 2, 2, 2, 2, 2, 1, 1, 1),
                           k = 2, local = !T), 1.298794, tolerance = 1e-6)			   
})

test_that("block_entropy on ensamble of series", {
  series      <- matrix(0, nrow = 8, ncol = 2)
  series[, 1] <- c(1, 1, 0, 0, 1, 0, 0, 1)
  series[, 2] <- c(0, 0, 0, 1, 0, 0, 0, 1)
  expect_equal(block_entropy(series, k = 2, local = !T), 1.788450, tolerance = 1e-6)
  ...
  expect_equal(block_entropy(series, k = 2, local = !T), 3.010976, tolerance = 1e-6)
})

test_that("block_entropy local on single series", {
  expect_equal(mean(block_entropy(c(1, 0, 0, 0, 0, 0, 0, 0, 0),
                           k = 2, local = T)), 0.543564, tolerance = 1e-6)
  expect_equal(mean(block_entropy(c(0, 0, 1, 1, 1, 1, 0, 0, 0),
                           k = 2, local = T)), 1.811278, tolerance = 1e-6)
  expect_equal(mean(block_entropy(c(1, 0, 0, 0, 0, 0, 0, 1, 1),
                           k = 2, local = T)), 1.548795, tolerance = 1e-6)
  expect_equal(mean(block_entropy(c(3, 3, 3, 2, 1, 0, 0, 0, 1),
                           k = 2, local = T)), 2.5, tolerance = 1e-6)
  expect_equal(mean(block_entropy(c(2, 2, 3, 3, 3, 3, 2, 1, 0),
                           k = 2, local = T)), 2.40564, tolerance = 1e-6)
  expect_equal(mean(block_entropy(c(2, 2, 2, 2, 2, 2, 1, 1, 1),
                           k = 2, local = T)), 1.298794, tolerance = 1e-6)   
})

test_that("block_entropy local on ensamble of series", {
  series      <- matrix(0, nrow = 8, ncol = 2)
  series[, 1] <- c(1, 1, 0, 0, 1, 0, 0, 1)
  series[, 2] <- c(0, 0, 0, 1, 0, 0, 0, 1)
